...
import { IChange, ITemplate, IRenamed, TColumn } from './types'
...
export const renderTemplateAdded = (added: IChange[], newTableString: TColumn): ITemplate => {
    const columns = tableToColumns(newTableString)
...
export const renderTemplateDeleted = (deleted: IChange[], oldTableString: TColumn): ITemplate => {
    const columns = tableToColumns(oldTableString)
    const ret: ITemplate = {up: '', down: ''}
...
export const renderTemplateUpdated = (updated: IChange[], oldTableString: TColumn, newTableString: TColumn): ITemplate => {
    const before = getTableAnalyzation(oldTableString)
    const now = getTableAnalyzation(newTableString)
